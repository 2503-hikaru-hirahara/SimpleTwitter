...
public class UserServiceTest {
	...
	public void setUp() throws Exception {
		...
		try {
			...
			ps.executeUpdate();
			//テストデータを投入する
			IDataSet dataSetBranch = new FlatXmlDataSet(new File("users_data_init.xml"));
	...
	public void testInsertUser() throws Exception {
		...
		try {
			...
			// 期待されるべきテーブルデータを表すITableインスタンスを取得
			IDataSet expectedDataSet = new FlatXmlDataSet(new File("users_data_insert.xml"));
			ITable expectedTable = expectedDataSet.getTable("users");
			...
			Assertion.assertEqualsIgnoreCols(actualTable, expectedTable,
					new String[] { "id", "created_date", "updated_date", "password" });
	...
	public void testUserUpdate() throws Exception {
		...
		try {
			...
			ITable actualTable = databaseDataSet.getTable("users");
			//期待されるべきテーブルデータを表すITableインスタンスを取得
			IDataSet expectedDataSet = new FlatXmlDataSet(new File("users_data_update.xml"));
			ITable expectedTable = expectedDataSet.getTable("users");
			//期待されるITableと実際のITableの比較
			//id、created_date、updated_date、passwordを除いたデータを確認
			Assertion.assertEqualsIgnoreCols(actualTable, expectedTable,
			...
			for (int i = 0; i < updateUserList.size(); i++) {
				if (!StringUtils.isBlank(updateUserList.get(i).getPassword())) {
					assertEquals(CipherUtil.encrypt((String) expectedTable.getValue(i, "password")),
